...
function sheet(title: string, headers: string[], rows: Cell[][], widths: number[], numeric: number[] = [], percent: number[] = [], leftAlignedDataRows: number[] = [], percentDataRows: number[] = [], forceLeftAligned = false) {
  ...
  const body = all.map((row, rowIndex) => {
    ...
    const style = forceLeftAligned
      ? rowIndex === 0 ? 6 : rowIndex === 1 ? 7 : percentDataRows.includes(rowIndex - 2) && columnIndex > 0 || percent.includes(columnIndex) ? 9 : numeric.includes(columnIndex) ? 8 : 5
      : rowIndex === 0 ? 1 : rowIndex === 1 ? 2 : leftAlignedDataRows.includes(rowIndex - 2) && columnIndex > 0 ? 5 : percentDataRows.includes(rowIndex - 2) && columnIndex > 0 ? 4 : percent.includes(columnIndex) ? 4 : numeric.includes(columnIndex) ? 3 : 0;
