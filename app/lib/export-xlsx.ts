...
export function exportHoldingsWorkbook(input: ExportInput) {
  ...
  const sheets = [
    ...
    sheet("数据源与口径", ["类别", "说明"], [["当前主数据源", "东方财富基金公开数据"], ["专业数据源", "iFind 尚未配置生产 API 授权，接入后可作为主源"], ["基金经理口径", "A/C 等份额持仓只计算一次，全部份额净资产合并；股票市值按定期报告披露值加总。"], ["更新机制", "网站每 6 小时刷新基金经理、在管关系、基金产品并探测最新财报期。"], ["用途", "客户基金经理季度持仓研究，不构成投资建议。"]], [24, 90]),
  ];
