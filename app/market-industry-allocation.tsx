...
export default function MarketIndustryAllocation({ period, query }: { period: string; query: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/market-industries?period=${encodeURIComponent(period)}`, { signal: controller.signal, cache: "no-store" })
      ...
      .then((payload) => {
        if (payload.period !== period || payload.scope?.classification !== "申万一级行业（2021）" || !Array.isArray(payload.industries)) {
          throw new Error("全行业数据口径不匹配");
        }
        if (!controller.signal.aborted) setData(payload);
      })
      .catch((reason) => {
        if (!controller.signal.aborted) setError(reason instanceof Error ? reason.message : "全行业数据读取失败");
      })
  ...
  return <section className="market-industry-module" aria-label="主动偏股公募基金申万一级行业配置">
    <div className="market-industry-title">
      <div>
        <span>ACTIVE EQUITY · SW LEVEL 1</span>
        <h2>全行业</h2>
        <p>普通股票型 + 偏股混合型 + 灵活配置型 + 平衡混合型 · 申万一级行业</p>
      </div>
    ...
    {loading ? <div className="loading-state"><span className="spinner" />正在读取主动偏股公募行业配置…</div> : error ? <div className="error-banner">{error}</div> : data ? <>
      <div className="market-industry-scope">
        <span><b>{fmt(data.activeFundCount, 0)}</b> 只主动偏股基金</span>
        <span><b>{data.industryCount}</b> 个申万一级行业</span>
        <span>行业识别覆盖 <b>{fmt(data.classificationCoverage * 100, 1)}%</b></span>
      </div>

      <div className="market-industry-toolbar">
        <div><strong>{query.trim() ? "匹配行业" : `${quarterLabel(period)}行业配置排名`}</strong><span>{rows.length} 条 · 按总市值降序</span></div>
      </div>

      {rows.length ? <div className="market-industry-table-wrap" tabIndex={0} role="region" aria-label="主动偏股公募基金申万一级行业配置表">
        <table className="market-industry-table compact-five">
          <thead><tr><th>排名</th><th>行业</th><th className="num">总市值</th><th className="num">{quarterLabel(period)}占比</th><th className="num">环比变动</th></tr></thead>
          <tbody>{rows.map((row) => <tr key={row.industryCode || row.industry}>
            <td><b className={`rank r${row.rank}`}>{row.rank}</b></td>
            <td><div className="market-industry-name"><strong>{row.industry}</strong><small>{row.industryCode}</small></div></td>
            <td className="num"><strong>{fmt(row.marketValue / 10_000, 1)}亿</strong></td>
            <td className="num accent">{fmt(row.allocationShare, 2)}%</td>
            <td className={`num ${row.qoqChange !== null && row.qoqChange > 0 ? "industry-up" : row.qoqChange !== null && row.qoqChange < 0 ? "industry-down" : ""}`}>{changeText(row.qoqChange)}</td>
          </tr>)}</tbody>
        </table>
